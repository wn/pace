...
//    var runs = [Run]()
    lazy var runs = {
        CachingStorageManager.default.persistentRealm.objects(Run.self).filter(filterPredicate)
    }()
    lazy var filterPredicate = {
        return NSPredicate(format: "runner.objectId == %@", argumentArray: [(user?.objectId ?? "")])
    }()
...
//        }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        CachingStorageManager.default.getRunsFor(user: user) { error in
            if let error = error {
                print(error.localizedDescription)
            }
        }
        runs.observe { [weak self] changes in
            switch changes {
            case .initial:
                break
            case .update:
