...
extension Array where Element == CheckPoint {
    /// Normalizes an array of checkpoints to produce an array of equidistant checkpoints.    
    var normalized: [CheckPoint] {
        return self.map { basePoint in
            basePoint.extractNormalizedPoint(from: self)
        }
    }
}
/*
